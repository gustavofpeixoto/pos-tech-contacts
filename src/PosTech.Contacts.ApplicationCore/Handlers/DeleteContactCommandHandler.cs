...
{
    public class DeleteContactCommandHandler(IAddContactRepository addContactRepository, IMessagingProducer messagingProducer) : IRequestHandler<DeleteContactCommand>
    {
        public async Task Handle(DeleteContactCommand request, CancellationToken cancellationToken)
        {
            Log.Information("Removendo contato. Id do contato: {contactId}", request.Id);

            await addContactRepository.DeleteContactAsync(request.Id);
